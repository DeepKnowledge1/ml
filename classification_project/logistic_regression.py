...
class IrisLogisticRegression:
    def __init__(self, file_path):
        ...
        self.data = []
        self.labels = []
        self.model = LogisticRegression(max_iter=200)  # Increase max_iter if needed <button class="citation-flag" data-index="9">
...
if __name__ == "__main__":
    ...
    logistic_regression_model = IrisLogisticRegression(file_path)
    
    # Load and preprocess data
    logistic_regression_model.load_data()
    logistic_regression_model.preprocess_data()
    ...
    prediction = logistic_regression_model.predict(sample)
    label_encoder = LabelEncoder()
    label_encoder.classes_ = np.load('classes.npy') if hasattr(label_encoder, 'classes_') else np.unique(logistic_regression_model.labels)
